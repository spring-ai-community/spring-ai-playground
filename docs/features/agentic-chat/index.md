...
![Agentic Chat workspace - the conversation area above the prompt input, with the reasoning, built-in tool, external MCP, and RAG document selectors on the selector row and the New Chat, Export, Prompt Library, and Settings actions in the header](../../assets/images/chat/chat-overview.png)
...
![The reasoning effort dropdown open on the selector row, showing Off, Low, Medium, and High](../../assets/images/chat/chat-reasoning.png)
...
![The tool selector popover - a Use built-in MCP server toggle, then multi-select boxes for Custom tools, Built-in tools to expose, and Composed external tools](../../assets/images/chat/chat-tool-selector.png)
...
![The chat settings drawer - sections for Model, System prompt, Generation, and Advanced details, with the Apply and New Chat button](../../assets/images/chat/chat-settings-drawer.png)

- **Model** - the section header names the active provider (`Model - Ollama` or `Model - OpenAI`) and lets you switch the chat model.
- **System prompt** - free-text, or pre-filled from a Prompt Library preset.
- **Generation** - temperature, top-p, max tokens, seed, and the per-chat **Memory (messages)** window - how many recent messages reach the model (see [Context Engineering → Conversation memory](../../context-engineering-architecture.md#conversation-memory)).
- **Advanced details** - stop sequences and a raw **provider-options JSON** editor for any option the form does not surface. It opens pre-expanded when it already holds a value.

![The Advanced details section expanded, showing the stop-sequence field and the provider-options JSON editor](../../assets/images/chat/chat-drawer-advanced.png)

The drawer is provider-aware: the option labels and the JSON placeholder change with the active provider. The full property mapping lives in [Context Engineering → Generation options](../../context-engineering-architecture.md#generation-options).

### Switching models and the download gate

The model dropdown marks any model that is not yet pulled into Ollama with a **download indicator**.

![The model dropdown with a download indicator on models that are not yet downloaded locally](../../assets/images/chat/chat-model-download-badge.png)

If you apply a model that is not installed, the chat does not start on a missing model. A gate dialog appears first; choosing **Download** pulls it with a live progress bar and a cancel option, and when the download finishes the chat starts on the new model.

![The model download gate dialog - a message that the model is not downloaded in Ollama yet, with Cancel and Download buttons](../../assets/images/chat/chat-download-gate.png)
...
![A conversation opened under a different provider - the input is disabled and a banner reads that the conversation was created with Ollama but the app is now running OpenAiSdk](../../assets/images/chat/chat-provider-mismatch.png)
...
![A rendered assistant turn - a highlighted Python code block with a copy button, inline and display math, a Mermaid flow diagram, and a small table](../../assets/images/chat/chat-rich-render.png)

### Message actions

Hovering a turn reveals its action bar - six controls left to right:

![The hover action bar on an assistant message - Collapse, Copy, Show raw, Read aloud, Quote in prompt, and Export](../../assets/images/chat/chat-action-bar.png)
...
![An agentic turn with an expanded THINK panel showing the model's reasoning and an expanded MCP TOOLS panel showing a getCurrentTime call with its request and result, above the final answer](../../assets/images/chat/chat-agentic-panels.png)
...
![The Export menu listing Markdown, Plain text, JSON, and PDF](../../assets/images/chat/chat-export-menu.png)
...
| **Memory** | Shared Agentic State | The full conversation is kept locally; each turn the model sees a configurable trailing window, supplied through `MessageChatMemoryAdvisor` over an [`LlmWindowChatMemory`](../../context-engineering-architecture.md#conversation-memory) decorator. | Agentic Chat drawer (per-chat **Memory (messages)**) + `spring.ai.playground.chat.memory-max-messages` (default 10); `history-max-messages` (2000) caps the local store | Coherent multi-turn dialogue without inflating every request; the recent-context window is tunable per conversation. | Models benefit from a longer window when the task needs more history. |
