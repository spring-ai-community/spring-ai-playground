  ...
  ai:
    playground:
      tool-studio:
        timeout-seconds: 30
        fs:
          base-path: ""    # blank -> ${user.home}/spring-ai-playground/workspace
        js-sandbox:
...
| `File Toolkit` | `getCurrentTime`, `evalExpression`, `listAllowedDirectories`, `readTextFile`, `listDir`, `grepFile`, `findFiles`, `sliceFile`, `sortFile`, `cutFileFields` | Filesystem read pipeline - reads anywhere under home, writes to the working directory; set `spring.ai.playground.tool-studio.fs.base-path` to relocate it (default `${user.home}/spring-ai-playground/workspace`) |
