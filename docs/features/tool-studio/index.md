...
- All 107 default tools are pure JavaScript executed through GraalVM Polyglot. No native dependencies, no per-OS build step, no install-time compile.
...
The app ships with a bundled catalog of **107 default tools** across six JSON source bundles. They are ready to call from chat the moment a model provider is connected, and they also serve as editable references when you start writing your own.
...
| `Everything` | All 107 default tools | Heavy MCP catalog |
