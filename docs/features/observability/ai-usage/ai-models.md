title: AI Models
description: Model and provider behaviour — latency distribution, throughput, finish reasons, streaming TTFT and per-chunk timings, embedding and image generation durations.

# AI Models

![AI Models dashboard — ten KPI cards (Total calls, Distinct models, Providers, Tokens/min, p50 latency, p99 latency, TTFT, Per-chunk, Embedding, Image) and charts for Calls per model, Tokens per model, Provider mix, Tokens over time, Provider HTTP requests by host, Finish reasons](../../../assets/images/observability/ai-models-full.png)
